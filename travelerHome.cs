...
namespace Travel_Ease
{
    public partial class travelerHome : Form
    {
                                    ...
                                    List<string> selectedActivities = null)
        {
         ...
         FROM Package p
         LEFT JOIN Accommodation ac ON p.AccommodationID = ac.AccomodationID
         LEFT JOIN Hotel h ON ac.HotelID = h.HotelID
         LEFT JOIN ServiceProvider sp_accom ON h.ProviderID = sp_accom.ProviderId
         LEFT JOIN Ride r on r.RideID = p.RideID
         LEFT JOIN TransportService ts ON r.TransportID = ts.TransportID
         LEFT JOIN ServiceProvider sp_transport ON ts.ProviderID = sp_transport.ProviderId
         LEFT JOIN Destination d ON p.DestinationID = d.DestinationID
         LEFT JOIN Location l ON d.LocationID = l.LocationID
         LEFT JOIN Operator o ON p.OperatorID = o.OperatorID
         WHERE 
             (@Keyword = '' OR p.Title LIKE '%' + @Keyword + '%' OR l.CityName LIKE '%' + @Keyword + '%' OR l.CountryName LIKE '%' + @Keyword + '%' OR d.Name LIKE '%' + @Keyword + '%')
            ...
            using (SqlCommand cmd = new SqlCommand(packageQuery, con))
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    tripCard.SetData(
                        ...
                        row["DestinationName"].ToString(), //added DestinationName
