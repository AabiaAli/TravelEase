...
namespace Travel_Ease
{
    public partial class Operator : Form
    {
        ...
        private readonly SqlConnection con = DbConnection.GetConnection();
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                reader.Close();
                destination.DataSource = destinations;
                destination.DisplayMember = "DisplayName";
                destination.ValueMember = "DestinationID";
        ...
        private void LoadRides()
        {
            try
            {
                using (var cmd = new SqlCommand("Select ServiceProvider.ProviderName, RideID, Ride.TransportID, Price, TimeRequested, ArrivalTime, RideStatus FROM Ride INNER JOIN TransportService ON TransportService.TransportID = Ride.TransportID INNER JOIN ServiceProvider ON ServiceProvider.ProviderId = TransportService.ProviderID", con))
                {
                    var da = new SqlDataAdapter(cmd);
                    var dt = new DataTable();
                    da.Fill(dt);
        ...
        private void accomodationgridview_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            int accId = Convert.ToInt32(row.Cells["AccomodationID"].Value);
            try
            {
...
WHERE PackageID = @PackageID AND OperatorID = @OperatorID;";
        ...
        //    cmd.Parameters.AddWithValue("@DestID", Convert.ToInt32(destination.SelectedValue));
            // Validate and add DestinationID
            if (destination.SelectedValue != null && int.TryParse(destination.SelectedValue.ToString(), out int destId))
            {
            ...
            else
            {
                ...
                throw new InvalidOperationException("Destination selection is required."); // Stop execution
            }
...
SELECT SCOPE_IDENTITY();";
        ...
        {
            this.Hide();
            TotalBookingAndRevenue reports = new TotalBookingAndRevenue();
            reports.OperatorID = this._operatorId;
            reports.Show();
        }
        ...
        {
            this.Hide();
            destinationPopularity reports = new destinationPopularity();
            reports.ID = this._operatorId;
            reports.userType = "Operator";
            reports.Show();
        }

        private void button3_Click(object sender, EventArgs e)
        {
            paymentViewReport payments = new paymentViewReport();
            payments.OpID = this._operatorId;
            payments.Show();
            this.Hide();
        }
        ...
        private void AddExistingItineraryButton_Click(object sender, EventArgs e)
        {
