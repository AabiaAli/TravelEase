...
namespace Travel_Ease
{
    ...
    {
        public int OpID { get; set; } //so that traveler is retained when returning home, can be changed to whoever's id's interface this report is in and you can set it using the object created
        /*for example:
         ...
         */
        const string reportPath = "Hotel_and_Transport.PaymentTransactionFraudReport.rdlc";
        ...
        private void paymentViewReport_Load(object sender, EventArgs e)
        {
        ...
        {
            Operator op = new Operator(this.OpID);
            op.Show();
            this.Hide();
        }
